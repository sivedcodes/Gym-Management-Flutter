...
class _RushChartState extends State<RushChart> {
  ...
  Widget build(BuildContext context) {
    final hours =
        List<int>.generate(FakeDb.dayEnd - FakeDb.dayStart, (i) => FakeDb.dayStart + i);
    final max = widget.counts.fold<int>(1,
        (m, c) => c > m ? c : m);
    final sel = _selected;
    return Column(
      ...
      children: [
        SizedBox(
          ...
          child: ListView.builder(
            ...
            padding:
                const EdgeInsets.symmetric(horizontal: AppSpace.l),
            itemCount: hours.length,
            itemBuilder: (_, i) {
              final h = hours[i];
              final c = h < widget.counts.length
                  ? widget.counts[h]
                  : 0;
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (uid != null) {
                ref
                    .read(fakeDbProvider)
                    .saveSlot(uid, session, from, to);
              }
              Navigator.pop(ctx);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content:
                        Text('Timing saved. Thanks!')),
              );
            },
